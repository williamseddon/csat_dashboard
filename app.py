...
def _symptom_presence_mask(df: Optional[pd.DataFrame]) -> pd.Series:
    if df is None or getattr(df, "empty", True):
        return pd.Series(dtype=bool)
    det_cols, del_cols = _get_symptom_col_lists(df)
    symptom_cols = [c for c in _dedupe_keep_order(det_cols + del_cols) if c in df.columns]
    if not symptom_cols:
...
def _count_rows_with_any_symptom(df: Optional[pd.DataFrame]) -> int:
    mask = _symptom_presence_mask(df)
    return int(mask.sum()) if not mask.empty else 0



def _product_specific_taxonomy_counts(
    delighters: Optional[Sequence[str]] = None,
    detractors: Optional[Sequence[str]] = None,
) -> Tuple[int, int]:
    del_list = _normalize_tag_list(list(delighters or []))
    det_list = _normalize_tag_list(list(detractors or []))
    universal_dels, universal_dets = _universal_neutral_catalog()
    universal_del_set = set(universal_dels)
    universal_det_set = set(universal_dets)
    product_dels = [label for label in del_list if label not in universal_del_set]
    product_dets = [label for label in det_list if label not in universal_det_set]
    return len(product_dels), len(product_dets)



def _workspace_status_chips(overall_df: pd.DataFrame, filtered_df: pd.DataFrame) -> List[Tuple[str, str]]:
    chips: List[Tuple[str, str]] = []
    all_delighters = list(st.session_state.get("sym_delighters") or [])
    all_detractors = list(st.session_state.get("sym_detractors") or [])
    product_del_count, product_det_count = _product_specific_taxonomy_counts(all_delighters, all_detractors)
    total_catalog = len(_normalize_tag_list(all_delighters)) + len(_normalize_tag_list(all_detractors))
    sym_source = str(st.session_state.get("sym_symptoms_source") or "none")
    if product_del_count or product_det_count:
        chips.append((f"Taxonomy ready · {product_det_count + product_del_count} product labels", "green"))
    elif total_catalog:
        source_label = "Universal-only catalog" if sym_source in {"none", "built-in", ""} else "Catalog loaded"
        chips.append((f"{source_label} · {total_catalog} total labels", "blue"))
    else:
        chips.append(("Taxonomy not set", "yellow"))

    view_total = int(len(filtered_df)) if isinstance(filtered_df, pd.DataFrame) else 0
    if view_total > 0:
        tagged_in_view = _count_rows_with_any_symptom(filtered_df)
        coverage = tagged_in_view / float(max(view_total, 1))
        coverage_label = f"Symptom coverage · {tagged_in_view:,}/{view_total:,} in view ({coverage * 100:.0f}%)"
        coverage_color = "green" if coverage >= 0.6 else ("indigo" if coverage > 0 else "gray")
        chips.append((coverage_label, coverage_color))

    if st.session_state.get("sym_export_bytes"):
        chips.append(("Export ready", "green"))
    else:
        chips.append(("Export auto-refreshes after runs", "gray"))

    if isinstance(overall_df, pd.DataFrame) and isinstance(filtered_df, pd.DataFrame) and len(filtered_df) < len(overall_df):
        chips.append((f"Filtered view · {len(filtered_df):,} of {len(overall_df):,}", "indigo"))
    return chips
...
def _taxonomy_editor_payload(delighters=None, detractors=None):
    source_dels = _coerce_tag_entries(delighters or [])
    source_dets = _coerce_tag_entries(detractors or [])
    payload = {
        "source_dels": source_dels,
        "source_dets": source_dets,
        "del_text": "\n".join(source_dels),
        "det_text": "\n".join(source_dets),
    }
    payload["signature"] = hashlib.sha1(
        json.dumps(
            {"delighters": source_dels, "detractors": source_dets},
            ensure_ascii=False,
            sort_keys=True,
        ).encode("utf-8")
    ).hexdigest()
...
def _clear_symptom_wizard_editor_state():
    for key in ("sym_ai_del_edit", "sym_ai_det_edit", "_sym_ai_editor_sig"):
        st.session_state.pop(key, None)


def _sync_symptom_wizard_editor_state(ai_result):
    payload = _taxonomy_editor_payload(
        (ai_result or {}).get("delighters", []),
        (ai_result or {}).get("detractors", []),
    )
    if st.session_state.get("_sym_ai_editor_sig") != payload["signature"]:
        st.session_state["sym_ai_del_edit"] = payload["del_text"]
        st.session_state["sym_ai_det_edit"] = payload["det_text"]
        st.session_state["_sym_ai_editor_sig"] = payload["signature"]
    return payload


def _taxonomy_editor_state(ai_result, *, del_text=None, det_text=None):
    payload = _taxonomy_editor_payload(
        (ai_result or {}).get("delighters", []),
        (ai_result or {}).get("detractors", []),
    )
    current_del_text = payload["del_text"] if del_text is None else str(del_text or "")
    current_det_text = payload["det_text"] if det_text is None else str(det_text or "")
    current_dels = _coerce_tag_entries(current_del_text)
    current_dets = _coerce_tag_entries(current_det_text)
    added_dels = [label for label in current_dels if label not in payload["source_dels"]]
    removed_dels = [label for label in payload["source_dels"] if label not in current_dels]
    added_dets = [label for label in current_dets if label not in payload["source_dets"]]
    removed_dets = [label for label in payload["source_dets"] if label not in current_dets]
    return {
        **payload,
        "current_dels": current_dels,
        "current_dets": current_dets,
        "del_text": current_del_text,
        "det_text": current_det_text,
        "added_dels": added_dels,
        "removed_dels": removed_dels,
        "added_dets": added_dets,
        "removed_dets": removed_dets,
        "added_count": len(added_dels) + len(added_dets),
        "removed_count": len(removed_dels) + len(removed_dets),
        "edited": bool(added_dels or removed_dels or added_dets or removed_dets),
    }
...
def _save_inline_symptom_edit(row_id, row, final_dets, final_dels, *, updated_reviews, processed_rows, detractors, delighters, notice_prefix="Updated"):
    ...
    export_suffix = " Export will be ready after the next run."
    original_bytes = st.session_state.get("_uploaded_raw_bytes")
    summary_obj = dataset_edit.get("summary") or (st.session_state.get("analysis_dataset") or {}).get("summary")
    st.session_state["sym_export_bytes"] = _prepare_symptom_export_bytes(
        edited_reviews,
        summary_obj=summary_obj,
        original_bytes=original_bytes,
    )
    if st.session_state.get("sym_export_bytes") is not None:
...
def _prepare_symptom_export_bytes(updated_df: pd.DataFrame, *, summary_obj=None, original_bytes=None):
    try:
        if original_bytes:
            return _gen_symptomized_workbook(original_bytes, updated_df)
        if summary_obj is not None:
            return _build_master_excel(summary_obj, updated_df)
    except Exception as exc:
        _log.warning("Could not prepare symptom export bytes: %s", exc)
    return None
...
def _activate_ai_taxonomy_result(tax_result, *, source="ai", preserve_draft=True, auto_applied=False):
    ...
    _clear_symptom_wizard_editor_state()
    if preserve_draft:
...
def _reset_workspace_state(*, reset_source=True):
    ...
    st.session_state["sym_scope_choice"] = "Missing both"
    st.session_state["sym_n_to_process"] = 10
    st.session_state.pop("sym_batch_size_run", None)
    st.session_state["_sym_recommended_setup_sig"] = None
    st.session_state["_sym_recommended_setup_auto"] = True
...
def _render_workspace_header(summary, overall_df, filtered_df, prompt_artifacts, *, source_type, source_label, filter_description, active_items):
    ...
    status_chips = _workspace_status_chips(overall_df, filtered_df)
    if status_chips:
        st.markdown(_chip_html(status_chips), unsafe_allow_html=True)
...
def _render_review_prompt_tab(*, settings, overall_df, filtered_df, summary, filter_description):
    ...
    with st.container(border=True):
        ...
        if st.button("▶️ Run Review Prompt", type="primary", use_container_width=True, disabled=run_disabled, key="prompt_run_btn"):
            ...
            try:
                ...
                export_notice = ""
                try:
                    _get_master_bundle(summary, updated, prompt_artifacts)
    ...
    if dl_ready:
        st.caption("Download is already prepared and refreshes when you rerun the prompt or click refresh.")
    else:
        st.caption("Prepare download if you want a fresh workbook right now.")
...
#  TAB: SYMPTOMIZER
# ═══════════════════════════════════════════════════════════════════════════════
SYMPTOMIZER_SCOPE_OPTIONS = [
    "Current filtered missing",
    "Missing both",
    "Any missing",
    "Current filtered reviews",
    "All loaded reviews",
]



def _symptomizer_row_key_series(df: Optional[pd.DataFrame]) -> pd.Series:
    if df is None or getattr(df, "empty", True):
        return pd.Series(dtype=str)
    if "review_id" in df.columns:
        return df["review_id"].astype(str).fillna("").str.strip()
    return pd.Series(df.index.astype(str), index=df.index, dtype="string")



def _recommended_symptomizer_setup(work: pd.DataFrame, filtered_df: pd.DataFrame, overall_df: pd.DataFrame) -> Dict[str, Any]:
    work_df = work.copy() if isinstance(work, pd.DataFrame) else pd.DataFrame()
    overall_count = int(len(overall_df)) if isinstance(overall_df, pd.DataFrame) else int(len(work_df))
    filtered_count = int(len(filtered_df)) if isinstance(filtered_df, pd.DataFrame) else 0
    if work_df.empty:
        return {
            "scope": "Any missing",
            "n_to_process": 0,
            "batch_size": 8,
            "in_scope": 0,
            "note": "No reviews are loaded yet.",
        }

    needs_del = pd.Series(False, index=work_df.index, dtype=bool)
    if "Needs_Delighters" in work_df.columns:
        needs_del = work_df["Needs_Delighters"].fillna(False).astype(bool)
    needs_det = pd.Series(False, index=work_df.index, dtype=bool)
    if "Needs_Detractors" in work_df.columns:
        needs_det = work_df["Needs_Detractors"].fillna(False).astype(bool)
    missing_any_mask = needs_del | needs_det
    missing_both_mask = needs_del & needs_det

    filtered_mask = pd.Series(False, index=work_df.index, dtype=bool)
    if isinstance(filtered_df, pd.DataFrame) and not filtered_df.empty:
        filtered_keys = set(_symptomizer_row_key_series(filtered_df).tolist())
        if filtered_keys:
            filtered_mask = _symptomizer_row_key_series(work_df).isin(filtered_keys)

    counts = {
        "Current filtered missing": int((filtered_mask & missing_any_mask).sum()),
        "Missing both": int(missing_both_mask.sum()),
        "Any missing": int(missing_any_mask.sum()),
        "Current filtered reviews": int(filtered_mask.sum()),
        "All loaded reviews": int(len(work_df)),
    }

    if 0 < filtered_count < overall_count and counts["Current filtered missing"] > 0:
        scope = "Current filtered missing"
        note = "Focused on the current filtered view and skips rows that already have both sides tagged."
    elif counts["Any missing"] > 0:
        scope = "Any missing"
        note = "Targets rows missing at least one side so you avoid re-processing fully tagged reviews."
    elif 0 < filtered_count < overall_count and counts["Current filtered reviews"] > 0:
        scope = "Current filtered reviews"
        note = "No missing rows were detected, so the current filtered view is queued as the next best demo set."
    else:
        scope = "All loaded reviews"
        note = "All loaded reviews are already tagged or no narrower view is active, so the full workspace is queued."

    in_scope = int(counts.get(scope, 0))
    if in_scope <= 25:
        n_to_process = in_scope
    elif in_scope <= 75:
        n_to_process = min(50, in_scope)
    else:
        n_to_process = min(100, in_scope)

    if scope == "Current filtered missing":
        target_sample = work_df.loc[filtered_mask & missing_any_mask].copy()
    elif scope == "Missing both":
        target_sample = work_df.loc[missing_both_mask].copy()
    elif scope == "Any missing":
        target_sample = work_df.loc[missing_any_mask].copy()
    elif scope == "Current filtered reviews":
        target_sample = work_df.loc[filtered_mask].copy()
    else:
        target_sample = work_df.copy()
    sample_words = pd.to_numeric(
        target_sample.get(
            "review_length_words",
            target_sample.get("review_text", pd.Series("", index=target_sample.index)).fillna("").astype(str).str.split().str.len(),
        ),
        errors="coerce",
    ).fillna(0)
    avg_words = float(sample_words.mean()) if not sample_words.empty else 0.0
    if avg_words <= 60:
        batch_size = 12
    elif avg_words <= 100:
        batch_size = 10
        ...
        batch_size = 8
    if in_scope > 0:
        batch_size = max(1, min(batch_size, in_scope))

    return {
        "scope": scope,
        "n_to_process": int(max(n_to_process, 0)),
...
def _mark_symptomizer_setup_manual() -> None:
    st.session_state["_sym_recommended_setup_auto"] = False



def _maybe_apply_recommended_symptomizer_setup(recommended_setup: Dict[str, Any]) -> None:
    rec = dict(recommended_setup or {})
    rec_sig = json.dumps(rec, sort_keys=True, default=str)
    prev_sig = st.session_state.get("_sym_recommended_setup_sig")
    allow_auto = bool(st.session_state.get("_sym_recommended_setup_auto", True)) or prev_sig is None
    if allow_auto and rec_sig != prev_sig:
        st.session_state["sym_scope_choice"] = rec.get("scope") or "Any missing"
        st.session_state["sym_n_to_process"] = int(rec.get("n_to_process") or 0) or 1
        st.session_state["sym_batch_size_run"] = int(rec.get("batch_size") or st.session_state.get("sym_batch_size", 8) or 8)
        st.session_state["_sym_recommended_setup_auto"] = True
    st.session_state["_sym_recommended_setup_sig"] = rec_sig
...
The tagger reads more than the main review body — it may use pros, cons, comments, title/body variants, or other visible fields when available. Highlights work best when the stored evidence is verbatim. They can still miss when the captured snippet was paraphrased, heavily normalized, split across multiple fields, or pulled from a fuzzy evidence match instead of one exact span. This build now recovers more verbatim spans during evidence validation and checks a broader set of visible review fields to make highlighting much more consistent.
    ...
    with sym_tabs[0]:
        if not api_key:
            ...
            # ── STEP 2: Review taxonomy + Approve ─────────────────────────
                ...
                # Optional edit
                editor_seed = _sync_symptom_wizard_editor_state(ai_result)
                with st.expander("✏️ Edit before running", expanded=False):
                    st.caption("These drafts refresh automatically whenever you generate a new taxonomy, so the first click always uses the latest labels.")
                    r1, r2 = st.columns(2)
                    with r1:
                        st.markdown("🟢 Delighters")
                        edited_del_text = st.text_area("Edit", value=editor_seed["del_text"], height=220, key="sym_ai_del_edit")
                    with r2:
                        st.markdown("🔴 Detractors")
                        edited_det_text = st.text_area("Edit", value=editor_seed["det_text"], height=220, key="sym_ai_det_edit")
                editor_state = _taxonomy_editor_state(ai_result, del_text=edited_del_text, det_text=edited_det_text)
                taxonomy_edited_in_wizard = bool(editor_state.get("edited"))
                edit_status_chips = [
                    (f"Delighters ready: {len(editor_state.get('current_dels', []))}", "green"),
                    (f"Detractors ready: {len(editor_state.get('current_dets', []))}", "red"),
                    ("Edited in review step" if taxonomy_edited_in_wizard else "Using generated labels", "yellow" if taxonomy_edited_in_wizard else "gray"),
                ]
                if taxonomy_edited_in_wizard and (editor_state.get("added_count") or editor_state.get("removed_count")):
                    edit_status_chips.append((f"Label changes: +{editor_state.get('added_count', 0)} / -{editor_state.get('removed_count', 0)}", "indigo"))
                st.markdown(_chip_html(edit_status_chips), unsafe_allow_html=True)
                ...
                if action_cols[0].button("✅ Approve & run symptomizer", type="primary", use_container_width=True, key="sym_approve_and_run"):
                    # Accept taxonomy
                    final_dels, final_dets = _prepare_active_taxonomy(
                        edited_del_text,
                        edited_det_text,
                ...
                with action_cols[1]:
                    with st.expander("More options", expanded=False):
                        if st.button("🔄 Regenerate taxonomy", use_container_width=True, key="sym_regenerate"):
                            _clear_symptom_wizard_editor_state()
                            st.session_state.pop("sym_ai_build_result", None)
                            st.session_state["sym_wizard_step"] = 1
                            st.session_state["_sym_wizard_force_step"] = 1
                            st.rerun()
                        if st.button("← Back to product step", use_container_width=True, key="sym_wizard_back"):
                            st.session_state["sym_wizard_step"] = 1
                            st.session_state["_sym_wizard_force_step"] = 1
                            st.rerun()
    ...
    recommended_setup = _recommended_symptomizer_setup(work, filtered_df, overall_df)
    _maybe_apply_recommended_symptomizer_setup(recommended_setup)
    filtered_review_mask = pd.Series(False, index=work.index, dtype=bool)
    filtered_review_keys = set(_symptomizer_row_key_series(filtered_df).tolist()) if isinstance(filtered_df, pd.DataFrame) and not filtered_df.empty else set()
    if filtered_review_keys:
        filtered_review_mask = _symptomizer_row_key_series(work).isin(filtered_review_keys)
    needs_del_mask = work["Needs_Delighters"].fillna(False).astype(bool)
    needs_det_mask = work["Needs_Detractors"].fillna(False).astype(bool)
    with st.container(border=True):
        st.markdown(
        ...
        st.markdown(
            _chip_html([
                (f"Recommended · {recommended_setup.get('scope', 'Any missing')}", "green" if st.session_state.get("_sym_recommended_setup_auto", True) else "blue"),
                (f"{int(recommended_setup.get('n_to_process', 0)):,} reviews", "indigo"),
                (f"Batch {int(recommended_setup.get('batch_size', st.session_state.get('sym_batch_size', 8) or 8))}", "gray"),
            ]),
            unsafe_allow_html=True,
        )
        st.caption(str(recommended_setup.get("note") or ""))
        cfg = st.columns([1.55, 1.0, 0.95, 0.85, 0.95])
        scope_choice = cfg[0].selectbox(
            "Review set",
            SYMPTOMIZER_SCOPE_OPTIONS,
            key="sym_scope_choice",
            on_change=_mark_symptomizer_setup_manual,
        )
        if scope_choice == "Current filtered missing":
            target_df = work[filtered_review_mask & (needs_del_mask | needs_det_mask)]
        elif scope_choice == "Missing both":
            target_df = work[needs_del_mask & needs_det_mask]
        elif scope_choice == "Any missing":
            target_df = work[needs_del_mask | needs_det_mask]
        elif scope_choice == "Current filtered reviews":
            target_df = work[filtered_review_mask]
        else:
            target_df = work
        max_reviews_in_scope = max(1, len(target_df))
        default_n = int(st.session_state.get("sym_n_to_process", recommended_setup.get("n_to_process") or min(100, max_reviews_in_scope)))
        default_n = min(max(1, default_n), max_reviews_in_scope)
        st.session_state["sym_n_to_process"] = default_n
        n_to_process = cfg[1].number_input(
            "Reviews to process",
            min_value=1,
            max_value=max_reviews_in_scope,
            value=default_n,
            step=1,
            key="sym_n_to_process",
            on_change=_mark_symptomizer_setup_manual,
        )
        batch_size_default = int(st.session_state.get("sym_batch_size_run", recommended_setup.get("batch_size") or st.session_state.get("sym_batch_size", 8)) or 8)
        ...
        using_recommended = (
            scope_choice == recommended_setup.get("scope")
            and int(n_to_process) == min(max(1, int(recommended_setup.get("n_to_process") or 1)), max_reviews_in_scope)
            and int(batch_size) == int(recommended_setup.get("batch_size") or batch_size)
        )
        if using_recommended:
            st.caption(f"Model: {_shared_model()} · Review set: {scope_choice} · Recommended setup loaded")
        else:
    ...
    if run_btn:
        ...
        sym_export_bytes = _prepare_symptom_export_bytes(
            updated_df,
            summary_obj=dataset.get("summary"),
            original_bytes=st.session_state.get("_uploaded_raw_bytes"),
        )
        st.session_state.update(
            ...
            sym_qa_user_edited=False,
            sym_qa_notice=None,
            sym_export_bytes=sym_export_bytes,
        )
        ...
        export_ready_suffix = " Export is ready." if sym_export_bytes is not None else ""
        status.success(
...
Keep it concise — each bullet should be 1-2 sentences max."""
    ...
    refresh_label = "🔄 Refresh export file" if sym_bytes is not None else "🧾 Prepare export"
    if ec1.button(refresh_label, use_container_width=True, key="sym_prep_export"):
        ...
        if sym_bytes is not None:
            ...
            st.warning("Export could not be prepared for this workspace yet.")
    ...
    base_caption = "Writes the final AI symptom tags (including QA edits) plus AI Safety · Reliability · Sessions into existing AI columns when present, otherwise it appends safe new columns without overwriting local symptomization."
    if qa_export_metrics:
        base_caption += f" Current QA accuracy: {float(qa_export_metrics.get('accuracy_pct', 100.0) or 0):.1f}%."
    if sym_bytes is not None:
        base_caption += " Export is already prepared and auto-refreshes after new runs and inline QA edits."
    else:
        base_caption += " Prepare or refresh the export here if you want a fresh download right now."
    ec2.caption(base_caption)
