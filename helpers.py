...
def _normalize_uploaded_df(raw, *, source_name="", include_local_symptomization=False):
    ...
    n["product_id"] = _app()._series_alias(w, ["Base SKU", "Product ID", "Product Id", "ProductId", "BaseSKU"])
    ...
    n["post_link"] = _app()._series_alias(w, ["Post Link", "URL", "Review URL", "Product URL"])
    n["rating"] = _app()._series_alias(w, _app().UPLOAD_RATING_ALIASES)
    n["submission_time"] = _app()._series_alias(w, _app().UPLOAD_DATE_ALIASES)
    n["content_locale"] = _app()._series_alias(w, ["Content Locale", "Locale", "Location", "Country"])
